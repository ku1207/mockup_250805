...
import { generateBannerImageWithGPT, GPTImageGenerationRequest } from '@/services/gptImageServiceB';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // GPT 이미지 B 생성 서비스 호출
    console.log('GPT 이미지 B 생성 서비스 호출 중...');
    const result = await generateBannerImageWithGPT(requestData);
    
    console.log('GPT 이미지 B 생성 결과:', {
    ...
    if (result.success) {
      ...
      return NextResponse.json({
        success: false,
        error: result.error || '이미지 B 생성에 실패했습니다.'
      }, { status: 500 });
    }

  } catch (error) {
    console.error('=== AI 이미지 B 생성 API 오류 ===');
    ...
    // 사용자 친화적인 에러 메시지 생성
    let userMessage = 'AI 이미지 B 생성 중 서버 오류가 발생했습니다.';
